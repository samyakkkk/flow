  ...
  request(request: DesktopAppActivationRequest): Promise<DesktopAppActivationResponse> {
    if (this.#closed) {
      return Promise.resolve(
        failure(request.requestId, "renderer-unavailable", `${BRAND.name} is shutting down.`),
      );
    }
      ...
      if (pending.dispatched) {
        this.#settle(
          failure(
            ...
            `The ${BRAND.name} window closed before it opened the project.`,
          ),
        );
      }
    }
  ...
  cancel(requestId: string): void {
    this.#settle(
      failure(
        requestId,
        "renderer-unavailable",
        `The command closed before ${BRAND.name} was ready.`,
      ),
    );
  }
