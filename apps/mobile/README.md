...
Display names and icons come from the root [`branding.json`](../../branding.json). See [deployment branding](../../docs/operations/branding.md) for enterprise overrides.

> [!WARNING]
> Flow Mobile is currently in development and is not distributed yet. If you want to try it out, you can build it from source.
...
- `development`: Expo dev client, installable side-by-side as `Flow Dev`
- `preview`: persistent internal preview build, installable side-by-side as `Flow Preview`
- `production`: store/release build as `Flow`

Run commands from `apps/mobile`.

Flow Connect is optional and disabled in a fresh clone. Public configuration belongs in the
