...
export function documentLifecycle(document: BrainDocumentSummary): string {
  ...
  return document.kind === "skill" ? "Reusable procedure" : "Lasting context";
}
      ...
      <DialogPopup className="sm:max-w-3xl">
        <DialogHeader>
          <div className="mb-2 flex items-center gap-2 pr-7 text-xs text-muted-foreground">
            ...
            {skill ? "SKILL.md" : document?.kind === "notes" ? "Conversation notes" : "Memory"}
  ...
  return (
    <>
      ...
      {(selectedKind
        ? [selectedKind]
        : compact
          ? (["memory", "skill"] as const)
          : (["skill", "memory"] as const)
      ).map((kind) => {
        ...
        const Icon = kind === "skill" ? FileCode2Icon : SparklesIcon;
        return (
          <section
            key={kind}
            aria-label={kind === "skill" ? (selectedKind ? "Auto-Skills" : "Skills") : "Memories"}
            ...
            <header className={`flex items-start gap-2 ${compact ? "px-2 py-2" : ""}`}>
              ...
              <div className="min-w-0 flex-1">
                <h2 className="text-sm font-medium">
                  {kind === "skill" ? (selectedKind ? "Auto-Skills" : "Skills") : "Memories"}
