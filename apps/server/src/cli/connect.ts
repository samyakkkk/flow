...
const authorizeCli = Effect.fn("cloud.cli.authorize")(function* (options: {
  ...
  const existing = yield* tokens.getExisting.pipe(
    Effect.catchTag("CloudCliCredentialRefreshError", () =>
      Console.log(
        `The stored ${BRAND.connectName} credential could not be refreshed; signing in again.`,
...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ? `Run \`t3 connect link\` to authorize and enable ${BRAND.connectName}.`
    : !status.desired
      ? `Run \`t3 connect link\` to enable ${BRAND.connectName}.`
      : !status.linked
        ? `Start ${BRAND.shortName} to provision the environment link and launch its managed tunnel.`
        : undefined;
...
const confirmRelayClientInstall = (version: string) =>
  Prompt.run(
    Prompt.confirm({
      message: `The ${BRAND.shortName} relay client is required for ${BRAND.connectName}. Download and install version ${version}?`,
      initial: false,
    }),
  );
...
const logCloudDisconnectFailure = (
  ...
  Effect.logWarning(`${BRAND.connectName} disconnect operation failed.`).pipe(
    Effect.annotateLogs({
      operation,
      clearAuthorization,
...
export const reportCloudDisconnectResults = Effect.fn("cloud.cli.report_disconnect_results")(
  function* (input: {
    ...
    if (input.liveResult.status === "failed") {
      ...
      yield* Console.warn(
        `${BRAND.connectName} is disabled, but the running server could not stop its tunnel.
Restart that server to stop the connector.`,
      );
    } else {
      yield* Console.log(`${BRAND.connectName} is disabled locally.`);
    }
...
const linkEnvironmentForConnect = Effect.fn("cloud.cli.link_environment")(function* (options: {
  ...
  if (!publishOnly) {
    ...
    if (Option.isNone(installed)) {
      yield* Console.log(
        `${BRAND.connectName} setup cancelled. The relay client was not installed.`,
      );
      return null;
    }
...
const connectLoginCommand = Command.make("login", {
  ...
  Command.withDescription(`Authorize the ${BRAND.connectName} CLI without enabling remote access.`),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        yield* Console.log(`${BRAND.connectName}
`);
...
const connectLinkCommand = Command.make("link", {
  ...
  Command.withDescription(`Authorize this environment for ${BRAND.connectName} on next start.`),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
...
`);
        ...
        if (linked) {
          ...
          yield* Console.log(
            flags.publishOnly
              ? `✓ Authorized${connectedAs(linked.identity)}\n\nNext\n  Start ${BRAND.shortName} to publish agent activity (no managed tunnel).`
...
const connectStatusCommand = Command.make("status", {
  ...
  Command.withDescription(`Show persisted ${BRAND.connectName} and relay client state.`),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        if (!enabled) {
          ...
          if (!linkedNow && (yield* CliState.readCliDesiredLinkMode) === "publish_only") {
            yield* CliState.setCliDesiredCloudLink(false);
            yield* Console.log(`Cancelled the pending publish-only ${BRAND.connectName} link.`);
          }
        ...
        if (yield* CliState.readCliDesiredCloudLink) {
          yield* Console.log(
            `A ${BRAND.connectName} link is already pending. Start ${BRAND.shortName} to finish provisioning it; publishing starts once it links.`,
          );
          return;
        }
        yield* CliState.setCliDesiredCloudLink(true, "publish_only");
        yield* Console.log(
          `Restart ${BRAND.shortName} to finish authorizing this environment to publish (no managed tunnel is created).`,
        );
      }),
    ),
  ),
...
const connectUnlinkCommand = Command.make("unlink", {
  ...
  Command.withDescription(`Disable ${BRAND.connectName} while retaining the stored authorization.`),
  Command.withHandler((flags) =>
...
const connectLogoutCommand = Command.make("logout", {
  ...
  Command.withDescription(`Disable ${BRAND.connectName} and clear the stored CLI authorization.`),
  Command.withHandler((flags) =>
...
export const connectCommand = Command.make("connect", {
  ...
  Command.withDescription(`Set up ${BRAND.connectName} for this machine.`),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
...
${BRAND.name} is set to run while you are logged in to this Mac. The server establishes the ${BRAND.connectName} link on startup.`
              : `
✓ Background service ready

${BRAND.name} is set to keep running after you log out. The server establishes the ${BRAND.connectName} link on startup.`,
          );
          return;
        }
        const serveCommand = yield* resolveCliCommand("serve");
