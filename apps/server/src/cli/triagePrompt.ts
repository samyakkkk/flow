...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your ${BRAND.name} triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A ${BRAND.name} user is \
...
${TRIAGE_PLAYBOOK.replaceAll("T3 Code", BRAND.name).replaceAll("https://github.com/pingdotgg/t3code", BRAND.links.repository)}`;
...
export const buildTriageContext = (input: TriageContextInput) => `# ${BRAND.name} triage context
