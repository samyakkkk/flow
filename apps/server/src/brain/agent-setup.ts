...
5. The startup hook supplies a Flow conversation handle. Call bind_session with that exact handle before remember; its reply names the conversation's notes document for get_entity. If the harness does not expose the handle to the model, report conversation-note access as unverified; never select the most recent session in a folder.
