...
export async function manageAllAgentIntegrations(
  input: {
    operation: "status" | "configure";
    stateDir: string;
    harnesses: readonly BrainHarness[] | null;
    projects: readonly { id: ProjectId; workspaceRoot: string; workspaceId: string | null }[];
  },
  manage = manageAgentIntegration,
): Promise<BrainAgentIntegrations> {
  const projects: BrainAgentIntegrations["projects"][number][] = [];
  const detected = new Set<BrainHarness>();
  if (!input.projects.length) {
    const status = await manage({
      operation: "status",
      stateDir: input.stateDir,
      folder: input.stateDir,
    });
    for (const harness of status.detected) detected.add(harness);
  }
  for (const project of input.projects) {
    let integration: BrainAgentIntegration | null = null;
    try {
      const base = { folder: project.workspaceRoot, stateDir: input.stateDir };
      integration = await manage({ ...base, operation: "status" });
      for (const harness of integration.detected) detected.add(harness);
      if (input.operation === "configure") {
        const harnesses = input.harnesses ?? integration.detected;
        if (project.workspaceId && harnesses.length) {
          integration = await manage({
            ...base,
            operation: "configure",
          ...
          integration = await manage({ ...base, operation: "remove" });
        }
      }
      projects.push({
      ...
      projects.push({
        ...
        workspaceId: project.workspaceId,
        integration,
        error: cause instanceof Error ? cause.message : "Could not configure coding tools.",
      });
    }
  }
  return { harnesses: [...(input.harnesses ?? detected)], detected: [...detected], projects };
}
...
export async function bindProjectWithAgentTools(
  ...
  for (const { folder, integration } of integrations) {
    if (
      ...
      (integration.workspaceId !== input.workspaceId || input.harnesses?.length === 0)
    )
  ...
  for (const { folder, integration } of integrations) {
    const harnesses =
      input.harnesses ?? (integration.configured ? integration.harnesses : integration.detected);
    if (harnesses.length)
      await manage({
        operation: "configure",
        folder,
