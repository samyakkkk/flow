...
/**
 * Earlier versions kept Cursor, Grok and OpenCode off unless they had been used, and saved that
 * computed `false` for everyone. Every agent is now on by default, so the stamp is cleared once.
 * A marker file records that it ran: an agent turned off afterwards is a real choice and stays off.
 */
const STAMPED_OFF_PROVIDERS = ["cursor", "grok", "opencode"] as const;

function clearStampedOffProviders(settings: ServerSettings): ServerSettings | undefined {
  const stamped = STAMPED_OFF_PROVIDERS.filter((driver) => !settings.providers[driver].enabled);
  ...
  return {
    ...
    providers: {
      ...settings.providers,
      ...Object.fromEntries(
        stamped.map((driver) => [driver, { ...settings.providers[driver], enabled: true }]),
      ),
    },
  };
}
...
const make = Effect.gen(function* () {
  ...
  const loadSettingsFromDisk = Effect.gen(function* () {
    ...
    const agentsOnMarker = `${settingsPath}.agents-on`;
    if (!(yield* fs.exists(agentsOnMarker).pipe(Effect.orElseSucceed(() => true)))) {
      const cleared = clearStampedOffProviders(settings);
      if (cleared) settings = cleared;
      // The marker follows the saved file, so an interrupted run simply repeats next time.
      yield* (cleared ? writeSettingsAtomically(cleared) : Effect.void).pipe(
        Effect.andThen(fs.writeFileString(agentsOnMarker, "")),
        Effect.ignore,
      );
    }
