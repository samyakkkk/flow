...
function makeProviderServiceLayer(
  input: {
    ...
    readonly captureBrainEvent?: ProviderServiceLiveOptions["captureBrainEvent"];
  ...
  const layer = it.layer(
    Layer.mergeAll(
      makeProviderServiceLive({
        ...(input.loadBrainContext ? { loadBrainContext: input.loadBrainContext } : {}),
        ...(input.captureBrainEvent ? { captureBrainEvent: input.captureBrainEvent } : {}),
      }).pipe(
        Layer.provide(NodeServices.layer),
...
for (const driverName of [
  ...
  const setup = makeProviderServiceLayer({
    ...
    captureBrainEvent: (_thread, event) =>
      Effect.sync(() => {
        captures.push(event.data);
      }),
