...
// Every agent is on by default, and an enabled agent is probed by launching its real CLI.
// These tests state which agents they exercise, so a developer's installed CLIs are never started.
const agentsOff = {
  providers: {
...
const encodedDefaultServerSettings = deepMerge(
...
function makeMutableServerSettingsService(
  initial: ContractServerSettings = decodeServerSettings(encodedDefaultServerSettings),
) {
...
it.layer(
  Layer.mergeAll(NodeServices.layer, ServerSettingsModule.layerTest(agentsOff), TestHttpClientLive),
)("ProviderRegistry", (it) => {
  ...
  describe("ProviderRegistryLive", () => {
    ...
    it.effect("never probes an agent the user turned off", () =>
      Effect.gen(function* () {
        const serverSettings = yield* makeMutableServerSettingsService(
          decodeServerSettings(
            deepMerge(encodedDefaultServerSettings, {
