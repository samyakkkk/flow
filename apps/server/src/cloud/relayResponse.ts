...
export function relayRequestError(cause: unknown) {
  return isRelayResponseError(cause)
    ...
    : new EnvironmentHttpInternalServerError({
        message: `Could not complete the ${BRAND.connectName} relay request. ${isHttpClientError(cause) ? `The relay request failed (${cause.reason._tag}).` : "The relay returned an unexpected response."} Check this machine's network connection and relay availability, then retry.`,
      });
}
...
function recoveryHint(error: RelayProtectedError): string {
  switch (error._tag) {
    case "RelayEnvironmentLinkLimitExceededError":
      return `Unlink an unused environment in ${BRAND.connectName}, then restart ${BRAND.name} on this machine.`;
    case "RelayAuthInvalidError":
      return `Run \`t3 connect login\` to check this machine's authorization. If the stored credential was revoked, sign out with \`t3 connect logout\`, then run \`t3 connect\` again. Restart ${BRAND.name} after signing in.`;
    case "RelayEnvironmentLinkProofExpiredError":
    case "RelayEnvironmentLinkProofInvalidError":
      return `Check this machine's date and time, update ${BRAND.name}, then restart it.`;
    default:
...
export const filterRelayResponse = Effect.fn("cloud.filter_relay_response")(function* (
  ...
  const message = Option.isSome(decoded)
    ? `${BRAND.connectName}: ${decoded.value.message}. ${recoveryHint(decoded.value)} Trace ID: ${decoded.value.traceId}.`
    : `${BRAND.connectName} relay returned HTTP ${response.status} without a recognized error response. Check relay access and any proxy or firewall restrictions, then restart ${BRAND.name}.${requestId}`;
