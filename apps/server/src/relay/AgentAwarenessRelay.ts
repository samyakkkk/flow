      ...
      switch (startupState) {
        case "waiting-for-link":
          yield* Effect.logInfo(
            `agent activity publishing standby; waiting for ${BRAND.connectName} link reconciliation`,
          );
          break;
        case "disabled":
          yield* Effect.logInfo(
            `agent activity publishing disabled by ${BRAND.connectName} configuration`,
          );
          break;
        case "enabled":
