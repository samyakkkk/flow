...
PRIORITIZATION UNDER THE BUDGET: when more candidates exist than the observation budget allows, the human's stated context — field reports ("I onboarded a user and X happened"), preferences, intent, rationale — OUTRANKS agent-inferred technical findings of similar durability. Returning ONLY agent_inferred observations from a session where the human stated something durable is a failure mode: the human's words are the scarcest signal in the transcript, and technical facts can be re-derived from code while the human's context cannot.
...
3b. AGENT PROPOSALS ARE NOT DECISIONS. A fix, pattern, or approach the agent proposed that the human never ratified in the transcript is at most kind "plan", phrased as proposed ("proposed, not yet confirmed: …"). Never write an unratified proposal with settled framing ("the correct approach is X", as a decision or constraint) — a future reader must be able to tell decided from suggested.
