...
For latest messages in a channel use query="type:thread channel:C012345 sort:recent" (replace C012345 with the actual channel ID, including the ID inside Slack's <#C012345> notation). channel:channel-name also works. Omit keywords to list recent messages; add keywords for a filtered search. Report the returned dates with their timezone (archive timestamps are UTC). Include actual returned Slack links in answer_md, not just a claim that links are available. These are indexed messages, so distinguish archive results from a live Slack fetch. If there are no results, say no indexed messages were found for that channel rather than claiming no Slack access.
