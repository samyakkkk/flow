...
test('setup preserves user settings, quotes paths, repairs idempotently, and removes its own entries', async t => {
  ...
  // Headless Claude cannot ask for these permissions while loading Brain context.
  for (const name of ['bind_session', 'get_chat_memories', 'list_skills', 'read_skill', 'read_document']) {
    assert.ok(settingsAfter.permissions.allow.includes(`mcp__flow-graph__${name}`));
  }
  for (const name of ['remember', 'correct_graph']) {
    assert.ok(!settingsAfter.permissions.allow.includes(`mcp__flow-graph__${name}`));
  }
...
test('Claude and Codex setup and removal preserve unrelated unfinished Copilot configuration', async t => {
  const f = await fixture(t);
  await fs.mkdir(join(f.folder, '.vscode'));
  const file = join(f.folder, '.vscode/mcp.json');
  const original = '{ unfinished user config';
...
test('MCP binds exact native sessions and rejects cross-conversation rebinding', async t => {
  ...
  async function conversation(session) {
    ...
    for (const name of ['bind_session', 'get_chat_memories']) {
      assert.deepEqual(replies[6].result.tools.find(tool => tool.name === name).annotations, { readOnlyHint: true, openWorldHint: false });
    }
  }
