...
export async function mcp(project) {
  // UI sessions already have a thread-bound Brain through t3-code. Keep the
  // installed server healthy but expose no duplicate tools or capture path.
  if (process.env.FLOW_SESSION_ID) {
    for await (const line of createInterface({ input: process.stdin })) {
      const input = JSON.parse(line);
      if (input.id === undefined) continue;
      const result = input.method === 'initialize'
        ? { protocolVersion: '2024-11-05', capabilities: { tools: {} }, serverInfo: { name: 'flow-graph', version: '1.0.0' }, instructions: 'Flow manages this conversation. Use the session-provided t3-code Brain tools; no bind_session is needed. Do not fall back to a folder binding.' }
        : input.method === 'tools/list' ? { tools: [] }
        : input.method === 'ping' ? {} : undefined;
      process.stdout.write(JSON.stringify({ jsonrpc: '2.0', id: input.id, ...(result ? { result } : { error: { code: -32601, message: 'Use the session-provided t3-code Brain tools in Flow.' } }) }) + '\n');
    }
    return;
  }
  ...
  const extra = [
    { name: 'bind_session', description: 'Bind this MCP connection to the exact Flow session handle emitted by the capture hook. Never guess a handle or use another conversation’s handle.', inputSchema: { type: 'object', properties: { session: { type: 'string' } }, required: ['session'], additionalProperties: false }, annotations: { readOnlyHint: true, openWorldHint: false } },
    { name: 'get_chat_memories', description: 'Read saved notes from this bound conversation. Requires bind_session first.', inputSchema: { type: 'object', properties: {}, additionalProperties: false }, annotations: { readOnlyHint: true, openWorldHint: false } },
  ];
