...
When running inside Flow, use the session-provided t3-code Brain tools as the
authority for the current project and conversation. Flow already binds and captures
that session: do not call bind_session or use a separate folder connection. If the
managed Brain tools are unavailable, report that; do not switch to another Brain.
The remaining instructions apply when running independently in a terminal or editor.
...
function renderClaude(ctx) {
  ...
  const readTools = ["orient", "find_entity", "get_entity", "read_query", "list_schema", "search_knowledge", "source_read", "source_search", "list_skills", "read_skill", "read_document", "bind_session", "get_chat_memories"].map(
