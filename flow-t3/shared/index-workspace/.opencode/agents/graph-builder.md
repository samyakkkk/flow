...
- On Repository nodes, `default_branch`, `remote`, `head_commit`, and `indexed_at` are orchestrator-managed freshness props stamped after every successful index — never set or edit them yourself; a model-written value would go stale and mislead every later session.
