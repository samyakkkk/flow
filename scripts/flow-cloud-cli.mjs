...
export function selectCloudRelease(releases, target = `${platform()}-${arch()}`) {
  if (!["darwin-arm64", "linux-x64"].includes(target))
    throw Error("Cloud CLI supports Apple Silicon macOS and Linux x64.");
  const eligible = releases.filter(
    (r) => /^flow-cloud-cli-v\d+\.\d+\.\d+$/.test(r.tag_name) && !r.draft && !r.prerelease,
  );
  const selected = eligible.reduce(
    (a, b) =>
      !a ||
      newerTag(
        b.tag_name.replace("flow-cloud-cli-", "flow-"),
        a.tag_name.replace("flow-cloud-cli-", "flow-"),
      )
        ? b
        : a,
    null,
  ...
  for (const [asset, url] of [
    [name, archiveUrl],
    [name + ".sha256", archiveUrl + ".sha256"],
  ])
    if (!selected.assets?.some((a) => a.name === asset && a.browser_download_url === url))
      throw Error(`Cloud CLI release is missing ${asset}.`);
  return {
    tag: selected.tag_name.replace("flow-cloud-cli-", "flow-"),
    archiveUrl,
...
export async function latestCloudRelease(fetcher = fetch) {
  const response = await fetcher(
    "https://api.github.com/repos/samyakkkk/flow/releases?per_page=100",
    { signal: AbortSignal.timeout(2500), headers: { "User-Agent": "Flow-Cloud-CLI" } },
  );
...
export async function main(args) {
  ...
  if (await fs.stat(shims).catch(() => null)) {
    ...
    for (const [from, to] of [
      ...
      ...["agent-connector.mjs", "capture-replay.mjs", "agent-home.mjs", "cloud-setup.mjs"].map(
        (name) => [name, name],
      ),
    ]) {
  ...
  if (args[0] === "install-bundle") {
    ...
    const publicBin = join(homedir(), ".local/bin");
    await fs.mkdir(publicBin, { recursive: true });
    try {
      await fs.writeFile(join(publicBin, "flow"), launcher, { mode: 0o755, flag: "wx" });
    } catch (e) {
      if (e.code !== "EEXIST") throw e;
    }
    console.log(
      `Cloud CLI installed: ${join(bin, "flow")}. Existing Flow commands and Mac apps are preserved.`,
    );
    return;
  }
  if (args[0] === "update") {
    await prepare(home);
    console.log("Cloud CLI updated. Run flow restart to apply a prepared server update.");
    return;
  }
  try {
    ...
    if (newerTag(latest.tag, installed.tag)) {
      ...
      const child = spawn(process.execPath, [self, "--prepare-update"], {
        detached: true,
    ...
    /* Offline update checks must not block installed commands. */
  }
  if (args[0] === "setup" && !args.includes("--cloud"))
    throw Error(
      "This CLI connects to Cloud Brains. Copy the setup prompt from your Cloud dashboard.",
    );
  if (args[0] === "setup" || args[0] === "agents") return runtimeMain(args);
  if (!args.length) {
    await runtimeMain(["--no-open"]);
    console.log(
      "Flow Cloud connector is running. Use your Cloud dashboard to connect project folders.",
    );
    return;
  }
  if (["status", "stop", "restart"].includes(args[0])) return runtimeMain([...args, "--no-open"]);
  throw Error(
    "Usage: flow | flow setup --cloud URL … | flow agents doctor|status|remove --folder PATH | flow update | flow restart",
  );
}
