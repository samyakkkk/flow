...
export async function main(args) {
  ...
  if (args[0] === "install-bundle") {
    ...
    console.log(
      `Flow CLI installed: ${join(bin, "flow")}. Existing Flow commands and Mac apps are preserved.`,
    );
    return;
  }
  if (args[0] === "update") {
    await prepare(home);
    console.log("Flow CLI updated. Run flow restart to apply a prepared server update.");
    return;
  }
  try {
