...
export async function supervise(directory) {
  ...
  try {
    ...
    if (config.mode !== "ui-only") {
      ...
      // Only the primary instance owns the user's coding-agent tool files. Not
      // on Windows yet: the hook commands Flow writes are POSIX shell, and a
      // broken hook in someone's Claude or Codex config is worse than none.
      if (config.name === "primary" && NodeOS.platform() !== "win32") env.FLOW_AGENT_TOOLS = "auto";
      if (releaseHome) {
