...
server.listen(port,'127.0.0.1',()=>void fetch(process.env.TEST_RECEIPT_URL,{method:'POST',body:JSON.stringify({pid:process.pid})}));
process.on('SIGTERM',()=>server.close(()=>process.exit(0)));
`;

async function fixture(t, { release = false, managed = false } = {}) {
  ...
  await NodeFSP.writeFile(join(code, "apps/server/src/bin.ts"), fakeServer);
  if (release) {
    ...
    await NodeFSP.symlink("releases/flow-v1.1.0", join(releaseHome, "current"));
  }
  ...
  return { root, directory, releaseHome, supervise, runtime, started: started.promise };
}
...
test(
  ...
  { timeout: 60000 },
  async (t) => {
    const { supervise, runtime, releaseHome } = await fixture(t, { release: true, managed: true });
    const supervisor = supervise();
    const state = await runtime();
