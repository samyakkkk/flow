...
async function fixture(t) {
  ...
  const config = { version: 1, id: 'environment', name: 'primary', mode: 'isolated', dev: false,
    home: join(directory, 'data'), code: join(home, 'releases/old-running-version') };
...
async function live(t, f, handler) {
  ...
  await f.save('runtime.json', { id: 'environment', generation: 'generation', token: 'private-control-token',
...
test('stopped service retains identity and pinned code without starting it', async t => {
  const f = await fixture(t); const result = await discoverCloudService(f.home);
  assert.equal(result.status, 'stopped'); assert.equal(result.environmentId, f.config.id);
...
test('authenticates live identity without exposing credentials or trusting response fields', async t => {
  ...
  await live(t, f, (req, res) => {
    ...
    res.end(JSON.stringify({ id: 'environment', generation: 'generation', phase: 'ready', token: 'leak', arbitrary: true }));
  });
  const result = await discoverCloudService(f.home);
  assert.equal(result.status, 'ready'); assert.equal(result.environmentId, 'environment');
  assert.equal(result.token, undefined); assert.equal(result.arbitrary, undefined);
...
test('rejects a reused port with a different service identity', async t => {
  const f = await fixture(t);
  await live(t, f, (_req, res) => res.end(JSON.stringify({ id: 'other', generation: 'generation', phase: 'ready' })));
  assert.equal((await discoverCloudService(f.home)).status, 'invalid');
});
test('does not follow redirects or treat authentication failure as stopped', async t => {
  const f = await fixture(t); let status = 302;
  await live(t, f, (_req, res) => { res.writeHead(status, { location: 'http://127.0.0.1:1/' }); res.end(); });
