...
export const T3ProjectFileScript = Schema.Struct({
  name: trimmedNonEmpty({
    description: `Display name for the script, shown in the ${BRAND.name} scripts menu.`,
  }),
  command: trimmedNonEmpty({
    description: `Shell command executed in a ${BRAND.name} terminal at the project root.`,
  }),
  ...
  description: `A project script that team members can import into ${BRAND.name}.`,
});
...
export const T3ProjectFile = Schema.Struct({
  ...
  iconPath: Schema.optionalKey(
    trimmedNonEmpty(
      {
        description: `Workspace-relative path to the project icon (e.g. "assets/logo.svg"). Checked before ${BRAND.name}'s built-in icon locations.`,
      },
  ...
  defaultThreadEnvMode: Schema.optionalKey(
    ThreadEnvMode.annotate({
      description: `Where new threads start for this repository: "worktree" for a fresh git worktree, "local" for the current checkout. A per-project setting in ${BRAND.name} overrides this; when neither is set, the global default applies.`,
    }),
  ),
  scripts: Schema.optionalKey(
    Schema.Array(T3ProjectFileScript)
      .annotate({
        description: `Project scripts shared with everyone who opens this repository in ${BRAND.name}.`,
      })
      .check(Schema.isMaxLength(T3_PROJECT_FILE_MAX_SCRIPTS)),
  ),
}).annotate({
  title: "T3 project file",
  description: `Checked-in project configuration for ${BRAND.name} (t3.json at the repository root). See https://t3.codes for documentation.`,
});
