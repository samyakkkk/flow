...
### Fixed — indexing failures are loud, classified, and say how to fix them
The graph is the first place a new user sees value, and its failure modes were silent: a signed-out Claude Code (it logs out frequently) failed every index job into a DB row nobody reads, the home page's `!lastIndexedCommit` heuristic showed "Flow is learning." **forever** after a failed first index, and a repo that indexed once then failed nightly showed "indexed". Now:
- **Failure classifier** (`orchestrator/src/indexer-failure.ts`): every index/enrich/correct job failure across all three CLIs (opencode/codex/claude) is classified into `{code, message, hint}` — signed-out CLI (with the per-CLI re-login command), credits/rate limit, CLI not installed, git clone denied (token hint), gateway/FalkorDB down, timeout, killed. Stored in the job's `result_json`, the `index_log` trail, and served by `/v1/repos/status` (`lastError`/`lastErrorCode`/`lastErrorHint`/`lastFailedAt`). Claude runs that exit 0 with `is_error` on the result line are failures now, not "done".
- **Environment preflight**: before spawning a CLI, the orchestrator probes the gateway's new `GET /health?deep=1` (which PINGs FalkorDB, bounded, never auto-creating a graph). A dead gateway or brain DB fails the job in seconds naming the broken layer — instead of 45 minutes later, or "succeeding" while every write bounced.
- **Status machine tells the truth**: `repoStatuses()` surfaces the latest REAL terminal outcome — a failure newer than the last success flips the repo to `failed` even if it indexed before (the UI still shows the last good index). Lifecycle bookkeeping rows (superseded/repo_removed/stalled) never masquerade as user-facing failures.
- **Dashboard shows it**: the home page drives "is indexing" off `/api/repos/status` (not commit presence), the Brain canvas renders a failed first index as "Indexing couldn't finish — <what broke>, <fix>" instead of building-forever, a live brain gets a "reindex failing, brain serves last good index" banner, Sources rows show the error + fix inline (was a hover tooltip), and `/api/health` marks the gateway unhealthy when FalkorDB is dead behind it.

### Fixed — `flow up` verifies FalkorDB instead of trusting the port, and recovers from squatted ports
`ensureFalkordb` adopted whatever answered on the FalkorDB port — a plain Redis (no graph module), a password-protected Redis, or any random service from an interrupted install "passed" boot, and every graph write then failed invisibly. Now whatever is on the port must answer RESP `PING` **and** `GRAPH.LIST` before Flow uses it (remote `FALKOR_HOST` targets are verified too). When the port is squatted by a non-FalkorDB (or allocated but unresponsive), `flow up` no longer demands the user free it: it starts `flow-falkordb-<port>` on a nearby free port, persists the choice as a machine default (`<data>/global.json` `FALKOR_PORT`), threads the resolved host/port into the gateway/orchestrator envs, and says exactly what it did. An explicitly pinned `FALKOR_PORT` still fails loudly rather than moving.

### Changed — opencode is no longer bundled; setup.sh installs it properly
The npm-bundled `opencode-ai` dependency is gone. Its platform binary ships **unsigned**, and macOS kills unsigned arm64 binaries at exec (SIGKILL, exit 137) — so the "works with nothing installed" fallback actually produced a mysterious postinstall failure on every Mac without a real opencode install. Now: indexing resolves the coding CLI from PATH only (opencode → codex → claude, as before), and if opencode is needed but missing, the job fails loudly with install instructions instead of silently spawning a binary the OS will kill. New **`setup.sh`** is the one-shot install on macOS/Linux/Windows(Git Bash/WSL2): deps (always `--include=dev` — `flow up` runs on tsx, and a stray `NODE_ENV=production` would silently strip it), Node 22+/Docker checks, CLI detection (skipping Flow-managed `node_modules` copies), auto-install of opencode through a signed channel when none is found — Homebrew or the official installer, npm only on Windows where there's no Gatekeeper; non-interactive shells (EC2 bootstrap) install without prompting — and registers `flow` on PATH, no separate `npm install -g .` step. For multi-branch dev, `setup.sh --alias flow-test-1 --branch <b>` clones that branch into its own managed checkout (`~/.flow/checkouts/<alias>`) with its own deps and data, registered as its own command; re-running fast-forwards it. Cold-install smoke test checkpoint 1c is inverted: it now fails if an opencode package ever reappears in `node_modules`.

### Added — branch notes + memory injection: Flow-side working memory that arrives unbidden
Memory you never maintain: a new `branch_notes` store in flow.db (never the user's repo — works identically local and EC2, migrates with export/import), scoped to (repo, branch), UNGATED by design (notes are attributed utterances, not truth claims). Three writers: an **auto rolling WIP note** per turn assembled free from the session's own output (title + agent's words + diff — zero LLM, zero discipline; supersedes itself so the branch always carries its current state), a session-safe **`note` verb** for in-the-moment discoveries/dead-ends/decisions (`{repo, branch}` are explicit args — Flow may be remote and never assumes the agent's filesystem), and humans via a new Inbox strip (view + delete).
**Turn-boundary injection**: every turn — first prompt and steers alike — gets a `[flow memory]` block with the top 3 relevant items (insert-mode procedures + this branch's notes), matched by one local embedding + in-process cosine, per-session dedup, 4s hard bound, no LLM in the path.
**Promotion**: after every base-branch reindex, notes graduate to graph `Note` nodes (new type + `ANNOTATES` edge; anchor hints resolved find-before-create) — wip is swept, note/caution/decision promote; feature branches promote via merge-commit detection, abandoned ones decay (30d). LLM distillation of transcripts is deliberately deferred to merge-time via the indexer (one cheap pass per merge, not per session).

### Security — memory lanes hardened (deliberately last, by priority call)
- **Correction verifiers are write-scoped**: a `correct_graph` verification job can only write to the flagged node ids (`FLOW_WRITE_SCOPE` enforced in the graph tools) — its prompt embeds agent-authored text, and full graph write access was a prompt-injection escalation path. Out-of-scope fixes must be rejected with a verdict, not applied.
- **Gateway HTTP is bearer-authed** when the project token is present (it is, under `flow up`): closes the localhost self-bless hole (`review_procedure` via curl) and is the prerequisite for remote-MCP/EC2 exposure. `/health` stays open; no token in env → dev-open with a loud warning. All internal callers (dashboard routes, orchestrator, indexer tools) send the bearer.
- **Permission cards show the payload**: the ACP permission prompt now renders the tool call's rawInput, so consenting to an external-lane proposal is no longer title-only. Also: `get_entity` stops shipping each node's 1536-float embedding vector in props (silent response bloat since embeddings landed).

### Added — procedures + graph corrections: agents can now contribute back, safely
Coding-agent sessions gain two governed proposal verbs on the flow-graph MCP — neither mutates the brain directly:
- **`propose_procedure`** — when a human states a durable rule mid-session ("always run migrations before deploy"), the agent drafts the COMPLETE procedure while it holds the context: name, why, a `trigger` when-clause (embedded for semantic retrieval), ordered steps, scope (repo/project), and delivery mode. It lands as a **`Procedure` node with `status: proposed`** — invisible to `find_entity` and unlinked until a human blesses it. New **Inbox** page in the dashboard reviews proposals: edit any field, approve (blesses *exactly* the reviewed text, materializes `GOVERNS` edges, re-embeds) or reject (node deleted; the journal keeps the record). Blessed procedures surface three ways: `GOVERNS` ambush on `get_entity` ("touching this node? read me first"), semantic `find_entity` on the trigger, and — for `mode: insert` rules — **automatic injection at session start** when the task prompt matches (budgeted at 2, strictly best-effort).
- **`correct_graph`** — an agent that finds graph content contradicting the code flags it (node ids + reason + file:line). The flag is ADVISORY: it lands in a new orchestrator `corrections` queue (overlapping flags coalesce), and a `correct_graph` job has the graph-builder verify it **only against the repo's registered base-branch checkout** — never the flagging agent's working copy — so branch-local and plain-wrong flags get rejected instead of polluting the shared brain. Verdicts (`applied`/`rejected`/`unclear`) show in the Inbox; unparsable outcomes land as `unclear`, never silently applied.
Schema: new `Procedure` node type + `GOVERNS` edge; the indexer is instructed never to create or clobber Procedures (they're human-blessed, reindexing can't verify them). Verified end-to-end on a scratch graph: propose → hidden → dedup gate (lexical + semantic) → approve-with-edits → GOVERNS materialized → retrievable; corrections: file → coalesce → job → verdict parsing.

Consent UX: `propose_procedure` is excluded from the session auto-approve gate, so the dashboard's existing permission prompt shows the full drafted procedure BEFORE the proposal is created — allow sends it to the Inbox, reject means nothing was ever written. (Also tightened that gate to exact tool-title matches; previously any tool call whose free-text title mentioned "flow-graph" — including bash commands — was silently auto-approved.)
