...
### Changed — incremental, repeat-safe chat memory checkpoints
Changed sessions now checkpoint every five minutes, including active sessions, and on closure. Each checkpoint freezes the full retained transcript and a new-event boundary; the model must cite real event IDs with new supporting evidence. Invalid output keeps the range pending. SQLite stores jobs and validated extraction results before application, stable observation IDs resume partial batches, and transactional consolidation prevents duplicate reinforcement after retries. Event evidence is counted once per memory/session/event. The processed cursor advances only when the entire batch succeeds. Existing `remember` behavior is unchanged; auto skills and rolling summaries remain separate work.
